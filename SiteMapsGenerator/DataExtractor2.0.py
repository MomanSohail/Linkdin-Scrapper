...
def get_text_from_url(url_link):
    ...
    try:
        ...
        return str(html_text).strip(),response.status_code

    except:
        "Sorry!Text Not Found.",response.status_code


def write_output_csv(file_name,data):

    header = ["Company_Name","Status","Company_Url","Text"]
...
def process_batch(output_file_name,i,df,testing):
    ...
    if not os.path.isdir(output_file_name):
        ...
        for company in df.iterrows():
            ...
            url_text,status=get_text_from_url(company[1]['Url'])

            if url_text!=None:
                raw_data_set.append([company[1]['Company_Name'],status,company[1]['Url'],url_text])
            if testing:
