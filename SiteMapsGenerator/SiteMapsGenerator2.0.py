...
def getFilesAndFolders(testing):
    ...
    input_batches_folder_name=os.path.join(directory_path,'data')
    output_folder_name=os.path.join(directory_path, "Output")
    
    test_input_file_name=None
    if testing:
        test_input_file_name=os.path.join(directory_path,'data','batch0_sitemap.csv')

    if not os.path.exists(input_batches_folder_name):
        os.mkdir(input_batches_folder_name)
    if not os.path.exists(output_folder_name):
        os.mkdir(output_folder_name)
    output_folder_name=os.path.join(output_folder_name, "SiteMaps")
    if not os.path.exists(output_folder_name):
        os.mkdir(output_folder_name)

    return input_file_name,input_batches_folder_name,output_folder_name,test_input_file_name


def get_batches(df,number_of_batches):

    batch_size=int(len(df)/number_of_batches)
    print(f"Number of Batches:{number_of_batches}||Batch Size:{batch_size}")
...
def save_batches(output_folder_name,batches):

    for i,b in enumerate(batches):
        output_file_name=os.path.join(output_folder_name,f"batch{i}_sitemap.csv")
        b.to_csv(output_file_name)
...
if __name__ == "__main__":
    ...
    input_file_name,input_batches_folder_name,output_folder_name,test_input_file_name=getFilesAndFolders(testing)
    
    df=pd.read_csv(input_file_name)
    df=df.dropna()

    batches=get_batches(df,number_of_batches)

    if not testing:
        save_batches(input_batches_folder_name,batches)

    os.chdir(input_batches_folder_name)
    for i,file in enumerate(os.listdir()):

        if test_input_file_name !=None:
            file=test_input_file_name
